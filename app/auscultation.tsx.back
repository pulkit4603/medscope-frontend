...
export default function AuscultationScreen() {
  ...
  const connectToDevice = async (device: BluetoothDevice) => {
    try {
      if (!bleManager) {
        Alert.alert("Bluetooth Not Available", "Cannot connect to device because Bluetooth is not available");
        return;
      }
  ...
  const useMockDevices = () => {
    ...
    setTimeout(() => {
      const mockDevices: BluetoothDevice[] = [
        { id: "00:11:22:33:44:55", name: "Mock Stethoscope", isConnected: false },
        { id: "AA:BB:CC:DD:EE:FF", name: "Mock AuscultTech 3000", isConnected: false },
        { id: "12:34:56:78:90:AB", name: "Demo Medical Device", isConnected: false }
      ];
