...
export default function PharyngoscopyScreen() {
  ...
  const [transformedImageBase64, setTransformedImageBase64] = useState<
    string | null
  >(null);
  ...
  const processCapturedImage = async (uri: string) => {
    try {
      // Don't set the captured image for display until we get the API response
      ...
      // Store the transformed image base64 for display
      if (result.transformed_image_base64) {
        setTransformedImageBase64(result.transformed_image_base64);
        // Set the captured image for display using the transformed base64 image
        setCapturedImageData(result.transformed_image_base64);
      } else {
        // Fallback to original image if no transformed image is available
      ...
      >
        ...
        {capturedImageData && !isProcessing && (
          <View style={styles.capturedImageContainer}>
            ...
            <Image
              source={{
                uri: capturedImageData.startsWith("data:")
                  ? capturedImageData
