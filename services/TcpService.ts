...
class TcpServer {
  ...
  getStatus(): { isRunning: boolean; address: string | null; port: number; clients: number } {
    return {
  ...
  // Event listeners
  on(event: 'connection' | 'data' | 'disconnection' | 'error', listener: (...args: any[]) => void): void {
    this.eventEmitter.on(event, listener);
  }

  off(event: 'connection' | 'data' | 'disconnection' | 'error', listener: (...args: any[]) => void): void {
